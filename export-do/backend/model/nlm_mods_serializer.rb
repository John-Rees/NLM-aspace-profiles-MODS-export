class MODSSerializer < ASpaceExport::Serializer
  ...
  def serialize_mods_inner(mods, xml)
	...
	#Log.info("rights: " + mods.rights.length.to_s)
    mods.rights.each do |right|
      right['acts'].each do |act|
        act['notes'].each do |note|
          serialize_note(note, xml)
        end
      end

      right['notes'].each do |note|
        serialize_note(note, xml)
      end
    end
        ...
        xml.roleTerm(:type => 'text', :authority => 'marcrelator') {
          if !name['role'].nil?
            if name['relator'].nil?
              ...
              xml.text (name['role'] + ". " + name['relator'])
            end
          end
        }
      }
